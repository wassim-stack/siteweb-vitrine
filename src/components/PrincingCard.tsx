...
interface PrincingCardProps extends Pricing {
  mode: PricingMode;
}

const PrincingCard: FC<PrincingCardProps> = ({
  annuallyPrice,
  description,
  features,
  monthlyPrice,
  ...
  return (
    <article
      className={`relative w-full py-6 px-4 flex flex-col space-y-4 mg:space-y-6 ${
        isMostPopular
    ...
    >
      ...
      <h1 className="tex-slate-100 font-bold text-base capitalize">{title}</h1>
      <p>{description}</p>
